...
IMG=96; BATCH=32; EPOCHS=8

# Akahana-only crops (PNG files with img_ prefix)
active = sorted([f for f in glob.glob("external_dataset/combined_cropped/active/*.png") if "img_" in f])
fatigue = sorted([f for f in glob.glob("external_dataset/combined_cropped/fatigue/*.png") if "img_" in f])
print("Akahana crops: %d active, %d fatigue" % (len(active), len(fatigue)))

files = [(f, 0) for f in active] + [(f, 1) for f in fatigue]
random.seed(42); random.shuffle(files)
val = files[:1500]; train = files[1500:]
...
def load(fl):
  ...
  for fp, l in fl:
    ...
    I.append(img.astype(np.float32))  # Already 96x96 from crop
    L.append(l)
...
print("Training on akahana-only crops...")
...
m.save("drowsiness_akahana_matched.keras")
print("Saved Keras")

conv = tf.lite.TFLiteConverter.from_keras_model(m)
conv.optimizations = [tf.lite.Optimize.DEFAULT]
tl = conv.convert()
open("drowsiness_akahana_matched.tflite", "wb").write(tl)
print("Saved TFLite: %.2f MB" % (len(tl)/1024/1024))

# Quick eval on val set
from tensorflow.lite.python.interpreter import Interpreter
ip = Interpreter(model_path="drowsiness_akahana_matched.tflite")
ip.allocate_tensors()
...
tt = tp+fp+tn+fn
print("TFLite val: %d/%d = %.2f%% P=%.4f R=%.4f F1=%.4f Sp=%.4f" % (c,tt,c/tt*100,tp/max(tp+fp,1),tp/max(tp+fn,1),2*tp/max(2*tp+fp+fn,1),tn/max(tn+fp,1)))
