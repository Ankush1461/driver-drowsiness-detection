# TRUE LOSO v7 - Optimized: k=30, 5-layer adapt, TTA, temporal smoothing
...
UNFREEZE_LAYERS = 25
K_SHOTS = 30  # labeled frames from test subject for adaptation
ADAPT_LR = 1e-4  # low LR for few-shot adaptation
ADAPT_EPOCHS = 10  # few epochs to avoid overfitting
...
def tta_predict(model, images, n_aug=5):
...
def main():
    ...
    print("="*70)
    print("TRUE LOSO v7 - Optimized: k=30, 5-layer adapt, TTA, temporal smoothing")
    print("="*70)
    subs = discover()
    ...
    print(f"TRUE LOSO v7 ({len(vs)} folds)\n")
    ...
    for fi, ts in enumerate(vs):
        ...
        adapt_probs = tta_predict(adapted_model, adapt_f, n_aug=5)
        best_thresh = find_best_threshold(adapt_probs, adapt_l)

        fp = tta_predict(adapted_model, eval_f, n_aug=5)
        fp = temporal_smooth(fp, window=5)
    ...
    print("\n" + "="*70)
    print("TRUE LOSO v7 RESULTS (mean +/- std)")
    print("="*70)
    ...
    cp = os.path.join(OUT, "uta_rldd_true_loso_v7.csv")
    ...
    fp2 = os.path.join(OUT, "uta_rldd_true_loso_v7_frames.csv")
    ...
    print("\n" + "="*70)
    print("LOSO v7: Optimized adaptive + TTA + temporal smoothing")
    print("="*70)
